    ...
    public partial class DatabaseSetupScreen : UserControl, IScreen
    {
        ...
        private void InitializeState()
        {
            if (m_state != null)
            {
                ...
                // *** For the LinearStateEstimator, defaulting to SQLite...
                ...
                if (migrate)
                {
                    object webManagerDir = Registry.GetValue("HKEY_LOCAL_MACHINE\\Software\\LinearStateEstimatorManagerServices", "Installation Path", null) ?? Registry.GetValue("HKEY_LOCAL_MACHINE\\Software\\Wow6432Node\\LinearStateEstimatorManagerServices", "Installation Path", null);
                    string configFile;

                    m_oldConnectionString = null;
                    m_oldDataProviderString = null;

                    // Attempt to use the LinearStateEstimator config file first
                    configFile = Directory.GetCurrentDirectory() + "\\LinearStateEstimator.exe.config";
                    ...
                    {
                        // Attempt to use the LinearStateEstimator Manager config file second
                        configFile = Directory.GetCurrentDirectory() + "\\LinearStateEstimatorManager.exe.config";
                        ...
                        {
                            // Attempt to use the web based LinearStateEstimator Manager config file as a last resort
        ...
        private void SqliteRadioButton_Checked(object sender, RoutedEventArgs e)
        {
            ...
            // *** For the LinearStateEstimator, not defaulting to sample data enabled...
            //if (!m_sampleScriptChanged && m_sampleDataScriptCheckBox != null)
