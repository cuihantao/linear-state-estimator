...
namespace ConfigurationSetupUtility.Screens
{
    ...
    public partial class SetupCompleteScreen : UserControl, IScreen
    {
        ...
        private ServiceController m_LinearStateEstimatorServiceController;
        ...
        {
            InitializeComponent();
            InitializeLinearStateEstimatorServiceController();
            ...
            get
            {
                ...
                if (m_state != null)
                {
                    try
                    {
                        ...
                        try
                        {
                            ConfigurationFile LinearStateEstimatorConfig = ConfigurationFile.Open("LinearStateEstimator.exe.config");

                            // Some of the crypto settings elements were renamed for consistency, remove the old ones
                            CategorizedSettingsElementCollection cryptoSection = LinearStateEstimatorConfig.Settings["cryptographyServices"];
                            cryptoSection.Remove("RetryDelayInterval");
                            cryptoSection.Remove("MaximumRetryAttempts");

                            // Example connection settings are updated between builds - remove the section and LinearStateEstimator will add back the latest
                            LinearStateEstimatorConfig.Settings.Remove("exampleConnectionSettings");

                            // Data publisher categories are now always lower case (such that code references are case insensitive)
                            LinearStateEstimatorConfig.Settings.Remove("dataPublisher");

                            LinearStateEstimatorConfig.Save(ConfigurationSaveMode.Full);
                        }
                        catch
                        {
                        ...
                        // If the user requested it, start or restart the LinearStateEstimator service
                        if (m_serviceStartCheckBox.IsChecked.Value)
                        {
                            try
                            {
#if DEBUG
                                Process.Start("LinearStateEstimator.exe");
#else
                                m_LinearStateEstimatorServiceController.Start();
#endif
                            }
                            catch
                            {
                                MessageBox.Show("The configuration utility was unable to start LinearStateEstimator service, you will need to manually start the service.", "Cannot Start Windows Service", MessageBoxButton.OK, MessageBoxImage.Information);
                            }
                        }

                        // If the user requested it, start the LinearStateEstimator Manager.
                        if (m_managerStartCheckBox.IsChecked.Value)
                        {
                            Process.Start("LinearStateEstimatorManager.exe");
                        }
                    }
                    finally
                    {
                        if (m_LinearStateEstimatorServiceController != null)
                            m_LinearStateEstimatorServiceController.Close();
                    }
                }
        ...
        public Dictionary<string, object> State
        {
            ...
            set
            {
                ...
                if (Convert.ToBoolean(m_state["restarting"]))
                    m_serviceStartCheckBox.Content = "Restart the LinearStateEstimator";
            }
        }
        ...
        // Initializes the LinearStateEstimator service controller.
        private void InitializeLinearStateEstimatorServiceController()
        {
            ServiceController[] services = ServiceController.GetServices();
            m_LinearStateEstimatorServiceController = services.SingleOrDefault(svc => string.Compare(svc.ServiceName, "LinearStateEstimator", true) == 0);
        }

        // Initializes the state of the LinearStateEstimator service checkbox.
        private void InitializeServiceCheckboxState()
        {
#if DEBUG
            bool serviceInstalled = File.Exists("LinearStateEstimator.exe");
#else
            bool serviceInstalled = m_LinearStateEstimatorServiceController != null;
#endif
            m_serviceStartCheckBox.IsChecked = serviceInstalled;
            m_serviceStartCheckBox.IsEnabled = serviceInstalled;
        }

        // Initializes the state of the LinearStateEstimator Manager checkbox.
        private void InitializeManagerCheckboxState()
        {
            bool managerInstalled = File.Exists("LinearStateEstimatorManager.exe");
        ...
        {
            string configFile = Directory.GetCurrentDirectory() + "\\LinearStateEstimator.exe.config";
