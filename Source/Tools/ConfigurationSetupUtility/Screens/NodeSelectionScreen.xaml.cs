...
namespace ConfigurationSetupUtility.Screens
{
    ...
    public partial class NodeSelectionScreen : UserControl, IScreen
    {
        ...
        private void UpdateDataGrid()
        {
            ...
            {
                m_infoTextBlock.Text = "Please select the node you would like the LinearStateEstimator to use.";
            }
            else
            {
            ...
            if (applyChangesToService)
                nodeId = GetNodeIdFromConfigFile("LinearStateEstimator.exe.config");
            else if (applyChangesToLocalManager)
                nodeId = GetNodeIdFromConfigFile("LinearStateEstimatorManager.exe.config");
            else
                nodeId = Guid.Empty;
            ...
            if (applyChangesToService)
                TryAddNodeInfo("LinearStateEstimator.exe.config");

            if (applyChangesToLocalManager)
                TryAddNodeInfo("LinearStateEstimatorManager.exe.config");
        ...
        private IDbConnection GetConnectionFromConfigFile()
        {
            ...
            string configFileName = Directory.GetCurrentDirectory() + "\\LinearStateEstimator.exe.config";
        ...
        {
            return GetValueOfSystemSetting("LinearStateEstimator.exe.config", "CompanyName");
        }
        ...
        // Occurs when the user selects a different node to be used by the LinearStateEstimator.
