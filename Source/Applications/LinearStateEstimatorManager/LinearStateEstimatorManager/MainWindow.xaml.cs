...
namespace LinearStateEstimatorManager
{
    /// <summary>
    ...
    public partial class MainWindow : ResizableWindow
    {
        ...
        private void ButtonHelp_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                Dns.GetHostEntry("LinearStateEstimator.codeplex.com");

                // Launch the help page available on web.
                Process.Start("http://LinearStateEstimator.codeplex.com/wikipage?title=Manager%20Configuration");
            }
            catch
            {
                useLocalHelp = true;
            ...
            if (useLocalHelp)
            {
                try
                {
                    ...
                    string helpfile = FilePath.GetAbsolutePath("LinearStateEstimatorManagerHelp.mht");

                    if (File.Exists(helpfile))
                        Process.Start(helpfile);
                    else
                        MessageBox.Show("Local help file \"" + helpfile + "\" was not found", "LinearStateEstimator Manager Help", MessageBoxButton.OK, MessageBoxImage.Warning);
                }
                catch (Exception ex)
                {
                    MessageBox.Show("Failed to launch local help file." + Environment.NewLine + ex.Message, "LinearStateEstimator Manager Help", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
